...
- **The calendar is live.** It opens on the real current month with the real current date marked. Mock deadlines anchor to day-of-month positions in the current month (tasks on the 5th and 9th, project window on the 23rd–24th), clamped to the month's length. Never hardcode an absolute date in `data/` or in a screen.
- **One clock.** All "now" reads go through `lib/today.ts`, which accepts an injectable `Date`. No component calls `new Date()` for calendar purposes.
...
A conflict scan across all 18 tasks (recorded in
`.superpowers/sdd/2026-08-21-halftone/progress.md`) found six places where two
tasks would have contradicted each other. Each was resolved and the resolution
is already folded into the task text below. They are restated here so a
reviewer or an executing agent can see *why* the plan reads the way it does,
and so a well-meaning "simplification" does not quietly reintroduce the bug.

| # | Ruling | Where it lands |
|---|---|---|
| R1 | `ONBOARDING_KEY` lives in `lib/storage.ts`, not as a named export from `app/index.tsx`. Route files export route contracts only. | Task 8, Step 4 |
| R2 | Every team in the mock data carries at least one message, so `threadPreview` is non-null per team. | Task 5, Steps 11–12 |
| R3 | `medium` / `semibold` / `bold` are removed from `tailwind.config.js` `fontFamily` and defined in `global.css` under `@layer components`, binding family and weight together. | Task 1, Steps 7–8 |
| R4 | No incoming proposal is seeded `status: 'accepted'`. | Task 5, Steps 11–12 |
| R5 | The "marks the real current date" test matches the day cell by regex, not exact string. | Task 15, Step 1 |
| R6 | `app/(tabs)/projects.tsx` wires real local save state instead of an inert `onToggleSave={() => {}}`. | Task 15, Steps 1 and 5 |

Rationale, in the same order:

- **R1** — an arbitrary named export from an expo-router route file couples a
  screen to a constant and invites a bundler warning. *If wrong:* a one-line
  import change, trivially reverted.
- **R2** — Task 11 asserts one preview row per team. A team with no messages
  renders none and fails the suite. *If wrong:* a few unused mock messages.
- **R3** — as originally written, Tailwind emits two competing rules for each of
  `font-medium` / `font-semibold` / `font-bold` (one `fontFamily`, one
  `fontWeight`) and which wins is undefined. expo-google-fonts registers each
  Inter weight as its own family, so weight alone cannot select a face on
  Android. *If wrong:* type renders at the wrong weight, visible immediately in
  Task 18's screenshot pass.
- **R4** — Task 16 asserts exactly one element reading "Accepted" after the user
  accepts one proposal. *If wrong:* one assertion loosens to `getAllByText`.
- **R5** — `MonthGrid` appends `, <kind> deadline` to a day that carries a mark,
  so an exact-string assertion fails on the 5th, 9th, 23rd and 24th. A test that
  passes 27 days a month is a latent break. *If wrong:* the assertion is
  marginally looser than intended.
- **R6** — an inert heart is a control that lies to the user, and reviewers will
  correctly flag it. *If wrong:* a few lines duplicating home's pattern.

One deliberate duplication survives the scan and is **not** a defect: `lib/calendar.ts`
defines its own `nextDay` rather than importing `dateRange` from `lib/derive.ts`,
which keeps the calendar math free of any dependency on the data layer. A
reviewer may flag it; the answer is that it is intentional.
...
/* Ruling R3. expo-google-fonts registers each Inter weight as its own family,
   so `font-weight` alone cannot select a face on Android. Binding family and
   weight together here — rather than as `fontFamily` keys in the Tailwind
   config — avoids emitting two competing rules for `.font-medium`,
   `.font-semibold` and `.font-bold` (one fontFamily, one fontWeight), where
   which one wins is undefined. Every component in this plan already uses these
   class names, so no call site changes. */
@layer components {
...
Only `display` and `sans` live here. `medium`, `semibold` and `bold` would
collide with Tailwind's own `font-medium` / `font-semibold` / `font-bold`
weight utilities — they are defined in `global.css` instead (Step 7, Ruling R3).
...
**Files:**
- Create: `halftone/data/types.ts`, `halftone/data/projects.ts`, `halftone/data/teams.ts`, `halftone/data/messages.ts`, `halftone/data/proposals.ts`, `halftone/data/profile.ts`, `halftone/lib/today.ts`, `halftone/lib/derive.ts`, `halftone/lib/format.ts`
- Test: `halftone/__tests__/lib/today.test.ts`, `halftone/__tests__/lib/derive.test.ts`, `halftone/__tests__/data/integrity.test.ts`

**Interfaces:**
- Consumes: nothing.
- Produces:
  - All types listed in Step 1.
  - `todayIso(now?: Date): ISODate`, `dayInCurrentMonth(day: number, now?: Date): ISODate`, `shiftDays(iso: ISODate, n: number): ISODate`, `monthOf(iso: ISODate): { year: number; month: number }`
...
- [ ] **Step 7: Write the failing clock test**
...
Run: `npx jest __tests__/lib/today.test.ts`
Expected: FAIL — cannot find module `../../lib/today`.

- [ ] **Step 9: Write `lib/today.ts`**
...
`todayIso` reads local calendar fields rather than slicing `toISOString()`, which would report yesterday for anyone west of UTC in the evening.

- [ ] **Step 10: Run the clock test and confirm it passes**

Run: `npx jest __tests__/lib/today.test.ts`
Expected: PASS, 11 tests.

- [ ] **Step 11: Write the mock data files**
...
- At least 6 projects. Two must be the comps' own: "Graphic design for new advertising campaign" ($500-600, Online/Remote, 3 Years EXP, Full time, Creative Minds Media) and "Sustainable City Design and Implementation" ($80 000 - $100 000, California, 5+ Years EXP, Full time).
- One project titled "Website Development" carrying a task "Design UI mockups for checkout process" due `dayInCurrentMonth(5)`, a second task due `dayInCurrentMonth(9)`, and a project window `dayInCurrentMonth(23)` → `dayInCurrentMonth(24)`. This reproduces comp 3's mark composition in whatever month the user opens the app.
- **All dates are computed, never literal.** Use `dayInCurrentMonth(n)` for deadlines and `shiftDays(todayIso(), -n)` for posted dates and message timestamps. A hardcoded `'2023-08-23'` anywhere in `data/` is a defect.
- Spread the other projects' deadlines across nearby day positions so the month has more than one project marked.
...
- **Every team carries at least one message** (Ruling R2). Task 11 renders one
  preview row per team from `threadPreview`, which returns `null` for a thread
  with no messages — a silent team would render nothing and fail that suite.
  The two teams outside comp 6 need only a single short message each.
- Proposals: at least two `incoming` and two `sent`, each pointing at a real
  project and a real `threadId`. **No incoming proposal is seeded
  `status: 'accepted'`** (Ruling R4) — Task 16 asserts exactly one element
  reading "Accepted" after the user accepts one, so a pre-accepted incoming
  record breaks it. Seed incoming records as `'sent'` or `'viewed'`; `'declined'`
  is fine. Sent records may carry any status.

Member counts are never written as a number — they come from `members.length`.

- [ ] **Step 12: Write the data integrity test**
...
- [ ] **Step 13: Run it and fix the data until it passes**

Run: `npx jest __tests__/data/integrity.test.ts`
Expected: PASS, 12 tests. Any failure means the mock data contradicts itself — fix the data, not the test.

- [ ] **Step 14: Guard against literal dates creeping back in**

Run: `grep -rnE "'20[0-9]{2}-[0-9]{2}-[0-9]{2}'" halftone/data/`
Expected: no output. Every date in `data/` must come from `lib/today.ts`.

- [ ] **Step 15: Commit**

```bash
git add halftone/data halftone/lib/today.ts halftone/lib/derive.ts halftone/lib/format.ts halftone/__tests__
git commit -m "Add typed mock data with a live clock and derived marks, agenda, and previews"
```
...
- Create: `halftone/app/onboarding.tsx`, `halftone/components/ui/Wordmark.tsx`, `halftone/lib/storage.ts`
- Test: `halftone/__tests__/screens/onboarding.test.tsx`

**Interfaces:**
- Consumes: `Halftone`, `PillButton`, `useTheme`.
- Produces: `<Wordmark size={number} />`, `ONBOARDING_KEY = 'halftone.onboarded'` (from `lib/storage.ts`), `SLIDES` array.
...
- [ ] **Step 4: Write `lib/storage.ts` and the real `app/index.tsx` splash**

Ruling R1: the onboarding flag lives in `lib/storage.ts`, not as a named export
from a route file. Route modules under expo-router should export only route
contracts — a default component and the router's own options — and importing an
arbitrary constant out of one couples a screen to its neighbour's internals.
...
```bash
git add halftone/app/index.tsx halftone/app/onboarding.tsx halftone/lib/storage.ts halftone/components/ui/Wordmark.tsx halftone/__tests__/screens
...
>
> The calendar is live: it opens on the real current month, and today's real date carries the `today` mark. `classifyDay` takes `todayIso` as a parameter rather than reading the clock itself, which is what keeps this module pure and testable.
...
**Interfaces:**
- Consumes: `monthGrid`, `classifyDay`, `projectRanges`, `rangePosition`, `marksFromProjects`, `agendaFromProjects`, `formatMonthYear`, `formatDayMonth`, `todayIso`, `monthOf`, `Segmented`, `Chip`, `ProjectCard`.
- Produces: nothing consumed by later tasks.
...
describe('My projects', () => {
  ...
  it('opens the calendar on the real current month', () => {
    goToCalendar();
    const now = new Date();
  ...
  it('toggles a project between saved and unsaved', () => {
    ...
    wrap();
    fireEvent.press(screen.getAllByLabelText(/save/i)[0]);
    expect(screen.getAllByLabelText(/Remove .* from saved/i).length).toBeGreaterThan(0);
  });
...
**Cross-task conflict scan.** A second pass compared every pair of tasks that share a file or an interface, and every task's tests against the code that same task specifies. Six contradictions surfaced and were resolved — see [Pre-flight Rulings](#pre-flight-rulings) above; all six resolutions are folded into the task text, and R2, R4 and R6 are additionally enforced by tests so a later edit cannot silently undo them.
