...
Expected: PASS, 10 tests. Any failure means the mock data contradicts itself — fix the data, not the test.
...
describe('My projects', () => {
  ...
  it('marks the real current date', () => {
    ...
    expect(screen.getByLabelText(String(today))).toBeTruthy();
  });
